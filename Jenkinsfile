pipeline {
  ...
  environment {
    GIT_REPO_URL = 'git@github.com:zchelalo/neuraclinic-file-management.git'
  ...
  stages {
    ...
    stage('Go pipeline') {
      ...
      stages {
        ...
        stage('Build') {
          steps {
            sh 'mkdir -p dist && go build -buildvcs=false -trimpath -o dist/neuraclinic-file-management ./cmd'
          }
          post {
            always {
    ...
    stage('Docker Build') {
      steps {
        sh 'docker build -f .docker/Dockerfile -t neuraclinic-file-management:${BUILD_NUMBER} .'
      }
    }
        ...
        withSonarQubeEnv(credentialsId: 'sonar-token', installationName: 'SonarQube') {
          sh '''
            ${SONAR_SCANNER_HOME}/bin/sonar-scanner \
              -Dsonar.projectKey=neuraclinic-file-management \
              -Dsonar.sources=. \
              -Dsonar.exclusions=gen/**,dist/**,certs/**,infra/** \
