...
ifneq ("$(wildcard $(LOCAL_PROTO_CONTRACTS)/buf.yaml)", "")
	cd $(LOCAL_PROTO_CONTRACTS) && buf generate \
		--template ../neuraclinic-file-management/buf.gen.yaml \
		--output ../neuraclinic-file-management \
...
build:
	mkdir -p dist
	go build -buildvcs=false -trimpath -o dist/neuraclinic-file-management ./cmd
